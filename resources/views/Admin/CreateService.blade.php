        ...
        <main>
            ...
            sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
                <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    <i data-lucide="plus-circle" class="w-6 h-6" style="color: #1ab79d;"></i>
                    Create a New Service
                </h2>

                <form action="{{ route('service.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    @csrf
                    ...
                    <div>
                       ...
                       @error('description') border-red-500 focus:ring-red-500 @else border-gray-300 focus:ring-[#1ab79d] @enderror"
                            placeholder="Describe your service..." required>{{ old('description') }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
